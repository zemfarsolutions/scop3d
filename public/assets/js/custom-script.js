...
async function ckAddSubscriber(post_data){var url=SITE_BASE_URL+'convertkit/add_subscriber.php'
...
return data;};async function ckAddIfNewGoogleUser(result){if(result.additionalUserInfo.isNewUser){var ckPayload={email:result.user.email,name:result.user.displayName,plan:'free_plan'}
return ckAddSubscriber(ckPayload);}
return 'Old User';}
async function verifyPlan(){var data=null;var url=SITE_BASE_URL+'verify-plan';$.ajax({url:url,type:"POST",async:false,success:function(resp){data=resp;},error:function(resp){data=resp.responseJSON;}})
return data;}
...
$('form').submit(function(){$(this).find('._disable_on_submit').prop('disabled',true);});function createDesignItem(data,showButton=null){showButton=true;isSelected=true;isShowUseAsInput=data.show_data;enhanceButton=true;showHdButton=true;precisionUpUser=data.precisionUserValue;showDelBtn=false;if(data.private==1){showDelBtn=false;}else{showDelBtn=true;}
...
function _createDesignItemBox(imageUrl,styles=[],canDownload=true,enhanceButton,isShowUseAsInput,isSelected,showHdButton,section,showDelBtn,precisionUpUser){var image=`<img class="rendered-img" src="${imageUrl}" alt="" loading="lazy">`;var icons='';if(canDownload){icons=`<div class="downld-bx">
                    ...
                    `;if(isShowUseAsInput==true){icons+=`<a class="sharetab sharetab-buttons use-as-input" data-img="${imageUrl}" href="javascript:void(0)" title="Use as Input">
						<img src="/web/images/input1.svg" alt="" loading="lazy">
						<span>Input</span>
					</a>`;if(enhanceButton==true){if(precisionUpUser=='true'){icons+=`<a class="sharetab sharetab-buttons ultra-enhancerbtn" onclick="showUpdateModal()" data-img="${imageUrl}" data-sec="${section}" href="javascript:void(0)" title="Ultra Enhancer">
									<img src="/web/images/enhance1.svg" alt="" loading="lazy">
									<span>Enhance</span>
								</a>`;}else{icons+=`<a class="sharetab  sharetab-buttons ultra-enhancer" onclick="ultraEnhancer(this)" data-img="${imageUrl}" data-sec="${section}" href="javascript:void(0)" title="Ultra Enhancer">
									<img src="/web/images/enhance1.svg" alt="" loading="lazy">
									<span>Enhance</span>
								</a>`;}}
if(showHdButton==true){if(precisionUpUser=='true'){icons+=`<div class="sharetab sharetab-buttons full_hd_quality share text-center" onclick="showUpdateModal()" data-img="${imageUrl}" data-sec="${section}" title="Full Hd Quality">
                                ...
                                <span>Precision+</span>
                                </div>`;}else{icons+=`<div class="sharetab sharetab-buttons full_hd_quality generate_hd_img share text-center" data-img="${imageUrl}" data-sec="${section}" title="Full Hd Quality">
									<img src="/web/images/hd1.svg">
									<span>HD</span>
...
icons+=`</div>`;if(isSelected==true){if(showDelBtn==true){var selectImage=`<div class="chkimg imgcheck">
...
function ipsPreviewImg(section){var file=$("#fileselect"+section)[0].files[0];let gallery=document.getElementById(`gallery${section}`);gallery.style.display='block';let uploadText=document.getElementById(`uploadText${section}`);uploadText.style.display='none';document.getElementsByClassName(`drop-cont${section}`)[0].style.visibility='hidden';let reader=new FileReader()
...
const ipsValidateImage=(file,success,fail,min_height_width=512)=>{var allowedFileType=['image/jpeg','image/png'];let fileTypeValid=allowedFileType.some((fileType)=>fileType===file.type);if(!fileTypeValid){return fail('type');}
var img=new Image();var oUrl=window.URL.createObjectURL(file);img.src=oUrl;var isValidDimension=false;return new Promise(res=>{img.onload=function(resp){var width=img.naturalWidth;var height=img.naturalHeight;window.URL.revokeObjectURL(img.src);isValidDimension=(width>=min_height_width&&height>=min_height_width);if(!isValidDimension){return fail('size');}
return success();};});};function updateFastSpring(userDetail=null){if(user){fastspring.builder.recognize({"email":user.email,});}}
function previewImage(src){$("#modalImagePreview").modal('show')
$("#mip").attr('src',src);}
var multipleDownloadImg=[];function getMultipleImages(src){if($('.ml_dw_img').is(':checked')&&!this.multipleDownloadImg.includes(src)){this.multipleDownloadImg.push(src);}else{this.multipleDownloadImg.splice(this.multipleDownloadImg.indexOf(src),1);}
...
function deleteMultipleImages(){var jsonData=JSON.stringify(this.multipleDownloadImg);var route=$('#deleteRenderImages').data('route');Swal.fire({title:'Are you sure?',text:"You won't be able to revert this!",icon:'warning',showCancelButton:true,confirmButtonColor:'#3085d6',cancelButtonColor:'#d33',confirmButtonText:'Yes, delete it!'}).then((willDelete)=>{if(willDelete.isConfirmed){$.ajax({url:route,type:"POST",data:{images:this.multipleDownloadImg},success:function(response){Swal.fire('Deleted!','Your Images has been deleted.','success')
setTimeout(function(){window.location.reload();},1000);},error:function(xhr,status,error){var error=error.responseJSON;Swal.fire('Oops!',error.message,'warning')}});}});}
...
function saveAs(blob,filename){if(typeof navigator.msSaveOrOpenBlob!=='undefined'){return navigator.msSaveOrOpenBlob(blob,fileName);}else if(typeof navigator.msSaveBlob!=='undefined'){return navigator.msSaveBlob(blob,fileName);}else{var elem=window.document.createElement('a');elem.href=window.URL.createObjectURL(blob);elem.download=filename;elem.style='display:none;opacity:0;color:transparent;';(document.body||document.documentElement).appendChild(elem);if(typeof elem.click==='function'){elem.click();}else{elem.target='_blank';elem.dispatchEvent(new MouseEvent('click',{view:window,bubbles:true,cancelable:true}));}
...
var page=1;$(document).on('click','.page-link',function(){page=$(this).attr('data-url').split('=').pop();getGeneratedDesigns('gallery');});function getGeneratedDesigns(type){if(get_designs_config.isLoading||get_designs_config.url==null||get_designs_config.page==null){return false;}
...
await fetch(aiAPI,{method:'POST',mode:'cors',cache:'no-cache',credentials:"include",headers:{accept:'multipart/form-data','Access-Control-Allow-Origin':'*'},crossDomain:true,body:formData,}).then(response=>{if(response.status==501){modalStore.style.display='block';}
return response.json();}).then(result=>{$('.ultra-enhancer').removeClass('disable-btn');$('.full_hd_quality').removeClass('disable-btn');$('._btn_gndeisgn').removeClass('disable-btn');$('.precision_btn').removeClass('disable-btn');genratedImage=result['Sucess']['generated_image'];orignalImage=result['Sucess']['original_image'];genratedImage.forEach(function(item){var data=document.getElementById(`all_data${sec}`);var newFreeformSpacer=document.createElement('div');data.insertBefore(newFreeformSpacer,data.firstChild);var design={original_url:orignalImage,generated_url:item,style:styleType,room_type:roomType,mode:modeType,show_data:true,section:sec,precisionUserValue:precisionUserValue,private:privatize}
var code=createDesignItem(design);var data=document.getElementById(`all_data${sec}`);document.getElementById(`progid`).style.display='none';var newFreeformSpacer=document.createElement('div');newFreeformSpacer.innerHTML=code;data.insertBefore(newFreeformSpacer,data.firstChild);});}).catch(error=>{console.error('Error:',error);});var progressindicatordiv=document.getElementById(`progressindicatordiv`);progressindicatordiv.remove();$(el).attr('disabled',false);_updateAiCatePillsStatus('enable');}
...
$('body').on('click','[data-download-url]',function(){var $url=$(this).data('download-url');download($url);});$('#skip').click(function(){$('#serveyModal').hide();});var ichecked=$("[name='question[0]']:checked").data('target-variant');$('#continue').click(function(){$('.survey_heading_div').addClass('hidden');var welcomeVariantEl=parseInt($(this).attr('welcome-active-variant'));var currentVariantEl=$(".welcome_content[welcome-variant='"+welcomeVariantEl+"']");currentVariantEl.removeClass('active');if(welcomeVariantEl==2){var firstVariantEl=$(".question_variant[data-variant='0']");var firstQuestionEl=$(".question-outer[data-question='1']");$('.servey_welcome').hide();$('#survey-next-button').show();firstVariantEl.addClass('active');firstQuestionEl.addClass('active');$('form#servey_form').css('padding','30px 40px');$('.questions-footer').show();$('.bottom-btn').hide();}
var continueButton=$("#continue");continueButton.attr('welcome-active-variant',2)
var currentVariantEl=$(".welcome_content[welcome-variant=2]");$('#skip').removeClass('hidden');currentVariantEl.addClass('active');$('.progress-start').val(25);var iframe=document.getElementById('welcome_video_frame');iframe.parentNode.removeChild(iframe);});function customInstruction(sec){var isCustomInstruction=document.getElementById(`nwcust${sec}`).checked;if(isCustomInstruction==true){$(`#custom_instruction${sec}`).show();}else{$(`#custom_instruction${sec}`).val('').hide();}}
let progressbarValue=25;$("#survey-next-button").click(function(){var activeVariantNo=parseInt($(this).attr('data-active-variant'));var activeQuestionNo=parseInt($(this).attr('data-active-question'));var currentVariantEl=$(".question_variant[data-variant='"+activeVariantNo+"']");if(activeVariantNo==0){var targetVariantNo=$("[name='question["+activeQuestionNo+"]']:checked").data('target-variant');var targetVariant=$(".question_variant[data-variant='"+targetVariantNo+"']");if(targetVariantNo==''||targetVariantNo==undefined||targetVariant.length==0){alert("Please select an answer before clicking Next.");progressbarValue=progressbarValue;$('.progrss-value').val(progressbarValue);return false;}
...
var formData=$('#servey_form').serialize();$.ajax({type:"POST",url:SITE_BASE_URL+"userServey",data:formData,success:function(result){alert('Thank you for your answers. We are personalizing your experience…');$('#serveyModal').modal('hide');location.reload();},error:function(error){console.log('error',error);}});});$("#survey-prev-button").click(function(){progressbarValue=progressbarValue-25;$('.progrss-value').val(progressbarValue);var submitButton=$("#survey-next-button");var activeVariantNo=parseInt(submitButton.attr('data-active-variant'));var activeQuestionNo=parseInt(submitButton.attr('data-active-question'));var currentVariantEl=$(".question_variant[data-variant='"+activeVariantNo+"']");$("#survey-next-button").html("Next");var currentQuestionEl=currentVariantEl.find('[data-question]').attr('data-question');var targetQuestionChecked=$("[name='question["+activeQuestionNo+"]']").prop("checked",false);var otherTextInput=currentVariantEl.find('[data-question="'+activeQuestionNo+'"] input.other_text');if(otherTextInput.length){otherTextInput.val('');}
...
var currentQuestion=$(".question-outer[data-question='"+activeQuestionNo+"']");var targetVariant=$(".question-outer[data-question='"+activeQuestionNo+"']").prev();currentQuestion.removeClass('active');targetVariant.addClass('active');submitButton.attr('data-active-variant',activeVariantNo);submitButton.attr('data-active-question',targetVariant.attr('data-question'));});function showOtherTextArea(radio,questionId){var otherTextArea=document.querySelector('input[name="other_text['+questionId+']"]');if(radio.checked&&radio.value==='other'){otherTextArea.style.display='block';otherTextArea.setAttribute('required','required');}else{otherTextArea.style.display='none';otherTextArea.removeAttribute('required');}}
async function ultraEnhancer(el){if(user==null){showLoginModal();return;}
...
newFreeformSpacer.innerHTML=`<div class="container22" id='progid'>
		...
		</div>`;data.insertBefore(newFreeformSpacer,data.firstChild);document.getElementById(`jumphere${sec}`).scrollIntoView();var divElement=document.getElementById(`all_data${sec}`);divElement.firstElementChild.scrollIntoView();var is_staging=(APP_LOCAL=='production')?'false':'true';_updateAiCatePillsStatus('disable');document.getElementById(`hundredid${sec}`).click();$.ajax({url:route,method:"POST",data:{"image":image_url},success:async function(resp){var formData=new FormData();formData.append("data",resp.data.image);var customInstruction=0;var roomType=resp.data.room_type;var strengthType='very_low';var modeType='Beautiful Redesign';var styleType='No Style';aiAPI=`${GPU_SERVER_HOST}/enhace?init=https://images.pexels.com/photos/1571460/pexels-photo-1571460.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1&id=${user.uid}&privateId=${resp.data.privateId}&is_staging=${is_staging}&roomtype=${roomType}&modeType=${modeType}&prompt=${styleType}&designtype=${sec}&is_custom_instruction=${customInstruction}&strengthType=${strengthType}`;await fetch(aiAPI,{method:'POST',mode:'cors',cache:'no-cache',credentials:"include",headers:{accept:'multipart/form-data','Access-Control-Allow-Origin':'*'},crossDomain:true,body:formData,}).then(response=>{if(response.status==501){modalStore.style.display='block';}
...
var code=createDesignItem(design);var data=document.getElementById(`all_data${sec}`);document.getElementById(`progid`).style.display='none';var newFreeformSpacer=document.createElement('div');newFreeformSpacer.innerHTML=code;data.insertBefore(newFreeformSpacer,data.firstChild);});}).catch(error=>{console.error('Error:',error);});var progressindicatordiv=document.getElementById(`progressindicatordiv`);progressindicatordiv.remove();$(el).attr('disabled',false);_updateAiCatePillsStatus('enable');},error:function(resp){data=resp.responseJSON;}})}
$(document).on('click','.generate_hd_img',function(){if(user==null){showLoginModal();return;}
...
newFreeformSpacer.innerHTML=`<div class="container22" id='progid'>
		...
		</div>`;data.insertBefore(newFreeformSpacer,data.firstChild);document.getElementById(`jumphere${sec}`).scrollIntoView();var divElement=document.getElementById(`all_data${sec}`);divElement.firstElementChild.scrollIntoView();_updateAiCatePillsStatus('disable');document.getElementById(`hundredid${sec}`).click();var image_url=$(this).data('img');$("#mip").attr('src',image_url);var route=$("#routeToFullHdImageData").data('route');var is_staging=(APP_LOCAL=='production')?'false':'true';$.ajax({url:route,method:"POST",data:{"image":image_url},success:async function(resp){var formData=new FormData();formData.append("data",resp.data.image);aiAPI=`${GPU_SERVER_HOST}/fullhd?init=https://images.pexels.com/photos/1571460/pexels-photo-1571460.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1&id=${user.uid}&privateId=${resp.data.privateId}&is_staging=${is_staging}&roomtype=${resp.data.room_type}&design_style=${resp.data.style}&modeType=${resp.data.mode}&roomtype=${resp.data.room_type}`;await fetch(aiAPI,{method:'POST',mode:'cors',cache:'no-cache',credentials:"include",headers:{accept:'multipart/form-data','Access-Control-Allow-Origin':'*'},crossDomain:true,body:formData,}).then(response=>{if(response.status==501){modalStore.style.display='block';}
...
var code=createDesignItem(design);var data=document.getElementById(`all_data${sec}`);document.getElementById(`progid`).style.display='none';var newFreeformSpacer=document.createElement('div');newFreeformSpacer.innerHTML=code;data.insertBefore(newFreeformSpacer,data.firstChild);}).catch(error=>{console.error('Error:',error);});},error:function(resp){data=resp.responseJSON;}})});function changeMode(sec){var modeValue=document.getElementById(`modeType${sec}`);var selectedOption=modeValue.options[modeValue.selectedIndex].value;var strengthValue=document.getElementById(`strength${sec}`);if(selectedOption==="Creative Redesign"){strengthValue.value="low";}else if(selectedOption==="Fill The Room"||selectedOption==="Fill The Exterior"||selectedOption==="Fill The Garden"){strengthValue.value="mid";$("#suggestionPrecisionModal").modal('show');}else if(selectedOption==="Beautiful Redesign"){strengthValue.value="mid";}}
function showUpdateModal(){$("#modalUpgradePlan").modal('show');}
function get_access_token(){var url=SITE_BASE_URL+'getTokenDetails'
$.ajax({url:url,type:"POST",async:false,success:function(resp){if(resp.success==true){let user_name=resp.data.name;let user_email=resp.data.email;aiAPI=`${API_GPU_SERVER_HOST}/get_token`;var payload={"name":user_name,"email":user_email,};fetch(aiAPI,{method:'POST',headers:{'Content-Type':'application/json',},crossDomain:true,body:JSON.stringify(payload),}).then(response=>{if(response.status==501){modalStore.style.display='block';}
return response.json();}).then(result=>{let access_Token=result.access_token;saveTokenToDatabase(user_email,access_Token);}).catch(error=>{console.error('Error:',error);});}else{alert(resp.error);}},error:function(resp){}})}
...
$(document).on('click','.edit_with_precision',function(){if(user==null){showLoginModal();return;}
...
var img=$(this).data('img');var routeURL=document.getElementById('editAsPrecision').getAttribute('data-route');$('.ultra-enhancer').addClass('disable-btn');$('.full_hd_quality').addClass('disable-btn');$('._btn_gndeisgn').addClass('disable-btn');$('.precision_btn').addClass('disable-btn');$.ajax({type:'POST',url:routeURL,data:{img:img},success:function(response){if(response&&response.b64image){var b64image='data:image/png;base64,'+response.b64image;sessionStorage.setItem('b64image',b64image);window.location.href='/user/precision';}},error:function(error){console.error('AJAX error:',error);}});});$("#closeserveyModal").click(function(){$("#serveyModal").hide();});$("#closeSuggestionPrecisionModal").click(function(){$("#suggestionPrecisionModal").hide();});$('document').ready(function(){if(window.innerWidth<991){initComparisons();}
else{var inkboxes=document.querySelectorAll(".inked-painted");var coloredBoxes=document.querySelectorAll(".colored");var fillerImages=document.querySelectorAll(".inked");for(var i=0;i<inkboxes.length;i++){inkboxes[i].addEventListener("mousemove",trackLocation.bind(null,i),false);inkboxes[i].addEventListener("touchstart",trackLocation.bind(null,i),false);inkboxes[i].addEventListener("touchmove",trackLocation.bind(null,i),false);}
function trackLocation(sectionIndex,e){var rect=fillerImages[sectionIndex].getBoundingClientRect();var position=((e.pageX-rect.left)/fillerImages[sectionIndex].offsetWidth)*100;if(position<=100){coloredBoxes[sectionIndex].style.width=position+"%";}}}});function initComparisons(){var x,i;x=document.getElementsByClassName("colored");for(i=0;i<x.length;i++){compareImages(x[i]);}
...
$("#closefeedbackModal").click(function(){$("#feedbackModel").hide();});$('#submitFeedbackRating').click(function(){var selectedRating=$('input[name="rate"]:checked').val();if(selectedRating==undefined||selectedRating==''){alert("Please select star to give rating!");return}
if(selectedRating<=3){$('.feedback_heading_div').hide();$('.bad_review').show();}
else{$('.feedback_heading_div').hide();$('.good_review').show();storeReviews();}});$("#submitFeedback").click(function(){var userFeedback=$('textarea[name="userfeedback"]').val();if(userFeedback==undefined||userFeedback==''){alert('Please fill the feedback!');return}
storeReviews();$("#feedbackModel").hide();});$(".trustpilot-widget").click(function(){$("#feedbackModel").hide();});function storeReviews(){var selectedRating=$('input[name="rate"]:checked').val();var userFeedback=$('textarea[name="userfeedback"]').val();$.ajax({type:"POST",url:SITE_BASE_URL+"userfeedback",data:{rating:selectedRating,feedback:userFeedback,},success:function(response){$('input[name="rate"]:checked').prop('checked',false);$('textarea[name="userfeedback"]').val('');},error:function(error){console.log('error',error);}});}